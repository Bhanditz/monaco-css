define('vs/language/css/emmet/expand/expand-full', ['exports'],function (exports) {
	'use strict';
	  ...
	  */
	   ...
	   */
			...
			value: function name(_name) {
				return strcase(_name, this.options.tagCase);
			}
	   ...
	   */
			...
			value: function isBooleanAttribute(attr) {
	   ...
	   */
			...
			key: 'field',
			value: function field(index, placeholder) {
	...
	var SnippetsStorage = function () {
		function SnippetsStorage(data) {
			_classCallCheck(this, SnippetsStorage);
			...
			value: function set(key, value) {
				...
				if (typeof key === 'string') {
					key.split('|').forEach(function (k) {
						return _this._string.set(k, new Snippet(k, value));
					});
			...
			value: function load(data) {
				...
				if (data instanceof Map) {
					...
					Object.keys(data).forEach(function (key) {
						return _this2.set(key, data[key]);
					});
				}
			}
	...
	var SnippetsRegistry = function () {
	  ...
	  */
		function SnippetsRegistry(data) {
			var _this3 = this;

			_classCallCheck(this, SnippetsRegistry);

			this._registry = [];

			if (Array.isArray(data)) {
				data.forEach(function (snippets, level) {
					return _this3.add(level, snippets);
				});
	  ...
	  */
		...
		_createClass(SnippetsRegistry, [{
			key: 'get',
			value: function get(level) {
	   ...
	   */
			...
			key: 'add',
			value: function add(level, snippets) {
				if (level != null && (typeof level === 'undefined' ? 'undefined' : _typeof(level)) === 'object') {
					snippets = level;
					level = 0;
				}
	   ...
	   */
			...
			value: function all(options) {
				...
				var fillResult = function fillResult(snippet) {
	...
	var Node = function () {
		...
		function Node(name, attributes) {
			...
			if (Array.isArray(attributes)) {
				attributes.forEach(function (attr) {
					return _this4.setAttribute(attr);
				});
			}
		}
	  ...
	  */
	   ...
	   * @return {Node}
	   */
			value: function create(name, attributes) {
	   ...
	   */
			...
			value: function replaceAttribute(curAttribute, newName, newValue) {
	   ...
	   */
			...
			key: 'removeClass',
			value: function removeClass(token) {
				token = normalize(token);
	   ...
	   */
			...
			key: 'insertBefore',
			value: function insertBefore(newNode, refNode) {
	   ...
	   */
			...
			value: function toString() {
				...
				var attrs = this.attributes.map(function (attr) {
					attr = _this5.getAttribute(attr.name);
	 ...
	 */
	var eatQuoted = function eatQuoted(stream, options) {
	 ...
	 * @return {Object}
	 */
	var consumeRepeat = function consumeRepeat(stream) {
	 ...
	 * position
	 */
	var consumeQuoted = function consumeQuoted(stream) {
	 ...
	 */
	var consumeAttributes = function consumeAttributes(stream) {
	 ...
	 * @return {Node}
	 */
	var consumeElement = function consumeElement(stream) {
	...
	function parse$2(str) {
		...
		while (!stream.eof()) {
			...
			if (ch === GROUP_START) {
				...
				var _node = new Node();
				groupStack.push([_node, ctx, stream.pos]);
				ctx = _node;
				stream.next();
				continue;
				...
				var _node2 = lastGroup[0];
				ctx = lastGroup[1];
				stream.next();

				// a group can have a repeater
				if (_node2.repeat = consumeRepeat(stream)) {
					ctx.appendChild(_node2);
				} else {
					// move all children of group into parent node
					while (_node2.firstChild) {
						ctx.appendChild(_node2.firstChild);
					}
				}
			...
			var node = consumeElement(stream);
	...
	var index$1 = function index$1(tree, registry) {
	...
	function mergeAttributes(from, to) {
		...
		for (var _i = 0, attr, a; _i < attrs.length; _i++) {
			attr = attrs[_i];
		...
		for (var _i2 = 0; _i2 < newAttrs.length; _i2++) {
			to.setAttribute(newAttrs[_i2]);
		}
	 ...
	 */
	var implicitTags = function implicitTags(tree) {
		tree.walk(function (node) {
	 ...
	 */
	var applyNumbering = function applyNumbering(tree) {
	...
	var blockCandidates1 = function blockCandidates1(className) {
		return (/^[a-z]\-/i.test(className)
		);
	};
	var blockCandidates2 = function blockCandidates2(className) {
		return (/^[a-z]/i.test(className)
		);
	};
	 ...
	 */
	var bem = function bem(tree, options) {
	 ...
	 */
	var addons = function addons(tree, _addons) {
		Object.keys(_addons || {}).forEach(function (key) {
			if (key in supportedAddons) {
				var addonOpt = _typeof(_addons[key]) === 'object' ? _addons[key] : null;
	 ...
	 */
	var index$2 = function index$2(tree, content, appliedAddons) {
		if (typeof content === 'string') {
			content = [content];
		} else if (content && (typeof content === 'undefined' ? 'undefined' : _typeof(content)) === 'object' && !Array.isArray(content)) {
	...
	function createModel(string) {
		...
		if (tokens.size) {
			...
			for (var i = 0, il = validMatches.length; i < il; i++) {
				var _token = validMatches[i];
				variables.push({
					name: _token[1],
					location: _token.index,
					length: _token[0].length
				});
			}
		}
	 ...
	 */
	function _mark(string, fields, token) {
		...
		var ordered = fields.map(function (field, order) {
			return { order: order, field: field, end: field.location + field.length };
		}).sort(function (a, b) {
	...
	function consumeField(stream, location) {
		...
		if (stream.eat(DOLLAR)) {
			...
			var _index = consumeIndex(stream);
			var _placeholder = '';

			// consumed $index placeholder
			if (_index != null) {
				return new Field(_index, _placeholder, location);
			}

			if (stream.eat(OPEN_BRACE)) {
				_index = consumeIndex(stream);
				if (_index != null) {
					if (stream.eat(COLON)) {
						_placeholder = consumePlaceholder(stream);
					}
	...
	var FieldString = function () {
		/**
	  * @param {String} string
	  * @param {Field[]} fields
	  */
		function FieldString(string, fields) {
		...
		_createClass(FieldString, [{
			key: 'mark',
			value: function mark(token) {
				return _mark(this.string, this.fields, token);
			}
		}, {
			key: 'toString',
			value: function toString() {
	...
	var defaultFieldsRenderer = function defaultFieldsRenderer(text) {
		return text;
	};
	...
	var OutputNode = function () {
		function OutputNode(node, fieldsRenderer, options) {
			_classCallCheck(this, OutputNode);

			if ((typeof fieldsRenderer === 'undefined' ? 'undefined' : _typeof(fieldsRenderer)) === 'object') {
	   ...
	   */
			...
			key: 'indentText',
			value: function indentText(text) {
				var _this6 = this;
				...
				return lines.map(function (line, i) {
					return i ? _this6.indent + line : line;
				}).join(nl);
			}
	   ...
	   */
			...
			key: 'renderFields',
			value: function renderFields(text) {
				return this._fieldsRenderer(text);
			}
		}, {
			key: 'toString',
			value: function toString(children) {
			...
			key: '_wrap',
			value: function _wrap(str, before, after) {
	 ...
	 * @return {String}
	 */
	var defaultField = function defaultField(index, placeholder) {
		return placeholder || '';
	};
	...
	function render(tree, field, formatter) {
		...
		var fieldsRenderer = function fieldsRenderer(text) {
	...
	function getFieldsModel(text, fieldState) {
		var model = (typeof text === 'undefined' ? 'undefined' : _typeof(text)) === 'object' ? text : parse$4(text);
		var largestIndex = -1;
	...
	function template(str, data) {
		...
		var stack = [];
		var replacer = function replacer(str, left, token, right) {
	...
	function handlePseudoSnippet(outNode) {
		...
		if (isPseudoSnippet(node)) {
			var fieldsModel = parse$4(node.value);
	...
	function shouldFormatNode(node, profile) {
		...
		if (node.parent.isTextOnly && node.parent.children.length === 1 && parse$4(node.parent.value).fields.length) {
	...
	var defaultAttrOptions = {
		primary: function primary(attrs) {
			return attrs.join('');
		},
	 ...
	 * @return {String}
	 */
	var index$3 = function index$3(tree, profile, syntax, options) {
	 ...
	 */
	var consumeColor = function consumeColor(stream) {
	...
	var Color = function () {
	  ...
	  */
		...
		_createClass(Color, [{
			key: 'toHex',
			value: function toHex(short) {
				var fn = short && isShortHex(this.r) && isShortHex(this.g) && isShortHex(this.b) ? toShortHex : _toHex;
	 ...
	 * @return {NumericValue}
	 */
	var consumeNumericValue = function consumeNumericValue(stream) {
		stream.start = stream.pos;
		if (eatNumber(stream)) {
	 ...
	 */
	var consumeKeyword = function consumeKeyword(stream, short) {
		stream.start = stream.pos;
	 ...
	 * @return {String}
	 */
	var consumeQuoted$1 = function consumeQuoted$1(stream) {
	 ...
	 * @return {Node}
	 */
	var index$4 = function index$4(abbr) {
		var root = new Node();
		var stream = new StreamReader(abbr);
		var node = void 0;

		while (!stream.eof()) {
			var _node3 = new Node(consumeIdent(stream));
			_node3.value = consumeValue(stream);
			...
			if (args) {
				...
				for (var i = 0; i < args.length; i++) {
					_node3.setAttribute(String(i), args[i]);
				}
			}
	 ...
	 */
	var stringScore = function stringScore(abbr, string) {
	 ...
	 * @return {CSSSnippet[]}
	 */
	var cssSnippets = function cssSnippets(snippets) {
		return nest(snippets.map(function (snippet) {
			return new CSSSnippet(snippet.key, snippet.value);
		}));
	};

	var CSSSnippet = function () {
		function CSSSnippet(key, value) {
			_classCallCheck(this, CSSSnippet);
			...
			value: function keywords() {
				...
				while (i < stack.length) {
					...
					if (item.value) {
						...
						for (var _j = 0, deps = item.dependencies; _j < deps.length; _j++) {
							if (stack.indexOf(deps[_j]) === -1) {
								stack.push(deps[_j]);
							}
						}
					}
	...
	function nest(snippets) {
		...
		for (var i = 0, cur, prev; i < snippets.length; i++) {
			cur = snippets[i];
	...
	var index$5 = function index$5(tree, registry, options) {
		...
		tree.walk(function (node) {
			return resolveNode$1(node, snippets, options);
		});
		return tree;
	};
	...
	function resolveAsProperty(node, snippet, formatOptions) {
		...
		if (node.value && _typeof(node.value) === 'object') {
			...
			if (!node.value.size) {
				...
				for (var i = 0, token; i < node.value.value.length; i++) {
	...
	function getScoringPart(item, key) {
		var value = item && (typeof item === 'undefined' ? 'undefined' : _typeof(item)) === 'object' ? item[key] : item;
		var m = (value || '').match(/^[\w-@]+/);
		return m ? m[0] : value;
	}
	...
	function tokenTypeOf(token, type) {
		return token && (typeof token === 'undefined' ? 'undefined' : _typeof(token)) === 'object' && token.type === type;
	}
	...
	function injectFields(string, values) {
		var fieldsModel = parse$4(string);
	...
	function stringifyAttribute(attr, options) {
		if (attr.value && _typeof(attr.value) === 'object' && attr.value.type === 'css-value') {
	 ...
	 * @return {String}
	 */
	var index$6 = function index$6(tree, profile, syntax, options) {
		if ((typeof syntax === 'undefined' ? 'undefined' : _typeof(syntax)) === 'object') {
			options = syntax;
			syntax = null;
		}
	 ...
	 * @return {Node}
	 */
	var index$8 = function index$8(node, options) {
	 ...
	 * @return {SnippetsRegistry}
	 */
	var snippetsRegistryFactory = function snippetsRegistryFactory(syntax, snippets) {
		...
		} else if ((typeof snippets === 'undefined' ? 'undefined' : _typeof(snippets)) === 'object') {
	...
	function parse$$1(abbr, options) {
		...
		return isStylesheet(options.syntax) ? parse$5(abbr, options) : parse$1(abbr, options);
	}
